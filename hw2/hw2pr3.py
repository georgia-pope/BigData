...
def linreg(X, y, reg=0.0):
	...
	identity[0,0] = 0 #looked at the solution for this part
	W_opt = np.linalg.solve(np.matmul(X.T, X) + reg*identity, np.matmul(X.T, y))
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	RMSE = np.linalg.norm((y - predict(W,X))/y.shape[0])
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	W_list = []
	"*** YOUR CODE HERE ***"
	reg_list = np.random.uniform(0,151, 150)
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list = np.sort(reg_list)
	W_list = [linreg(X_train, y_train, reg) for reg in reg_list]
	norm_list = np.linalg.norm(W_list, axis=1)
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	n_identity = np.eye(X.shape[1])
	m_identity = np.eye(X.shape[0])
	onesCol = np.ones((X.shape[0],1))
	W_opt = np.linalg.inv(X.T@(m_identity - (1/X.shape[0])*onesCol@onesCol.T)@X + reg*n_identity)@X.T@(m_identity - (1/X.shape[0])*onesCol@onesCol.T)@y

	b_opt = onesCol.T@(y-X@W_opt)/X.shape[0]
	"*** END YOUR CODE HERE ***"
...
#helper to find gradient of W
def grad_w(X, y, W, b, reg):
	grad_w = -(1/X.shape[0])*((X.T)@(y - X@W -b) + reg*W)
	return grad_w

#helper to find gradient of b
def grad_b(X, y, W, b):
	grad_b = 0
	grad_b = -(1/X.shape[0]) * (y - X @ W - b)
	return grad_b
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		train_rmse = np.sqrt((np.linalg.norm((y_train - predict(W,X_train) - b))**2/y_train.shape[0]))
		val_rmse = np.sqrt((np.linalg.norm((y_val - predict(W, X_val) - b))**2 / y_val.shape[0]))

		obj_train.append(train_rmse)
		obj_val.append(val_rmse)

		W_grad = grad_w(X_train, y_train, W, b, reg)
		b_grad = np.sum(grad_b(X_train, y_train, W, b))
...
if __name__ == '__main__':
	...
	onesColTrain = np.ones_like(X_train[:,0])
	onesColTrain.resize(onesColTrain.shape[0],1)
	X_train = np.hstack((onesColTrain, X_train))

	onesColVal = np.ones_like(X_val[:, 0])
	onesColVal.resize(onesColVal.shape[0], 1)
	X_val = np.hstack((onesColVal, X_val))

	onesColTest = np.ones_like(X_test[:, 0])
	onesColTest.resize(onesColTest.shape[0], 1)
	X_test = np.hstack((onesColTest, X_test))

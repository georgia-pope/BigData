...
if __name__ == '__main__':
	...
	img = ndimage.imread('clown.jpg', flatten=True)
	...
	"*** YOUR CODE HERE ***"
	t = np.linspace(0,k,k)
	orig_S_plot = plt.plot(t, S[0:k], "b-", label = "orig_S_plot")
	shuf_S_plot = plt.plot(t, S_s[0:k], "r.", label = "shuf_S_plot")


	"*** END YOUR CODE HERE ***"

	plt.legend(('original', 'shuffled'), loc = 'best')
	...
	for index in range(len(rank_list)):
		...
		"*** YOUR CODE HERE ***"
		identity = np.eye(k,k)
		S_diag = np.diag(S[0:k])
		img_k = U[:,0:k]@S_diag@V[0:k][:]
		plt.imshow(img_k, cmap = 'Greys_r')

...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			dist = np.linalg.norm(X[i] - clusters, axis=1)
			cluster_ind = np.argmin(dist)
			label[i] = cluster_ind
		for c in range(k):
			c_labels = np.where(label == c)[0]
			cluster_c = X[c_labels]
			cluster_sum = np.sum(cluster_c, axis = 0)
			cluster_num = cluster_c.shape[0]
			if cluster_num != 0:
				clusters[c] =  cluster_sum/cluster_num
...
def k_means_cost(X, clusters, label):
	...
	cost = 0
	for i in range(m):
		cost += np.linalg.norm(X[i] - clusters[label[i]])
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	plt.plot(np.linspace(1,21,20,endpoint=True), cost_k_list)
	...
	center_plot, = plt.plot(clusters[:,0], clusters[:,1], 'rD')

...
#this returns a m x 10 where m is the number of observations
def softmax(X, W):
	#might need to reshape
	#used solution to try to speed up time
	mu = X@W
	mu_exp = np.exp(mu)
	phi = mu_exp/ mu_exp.sum(axis = 1).reshape(-1, 1)
	return phi
...
def NLL(X, y, W, reg=0.0):
	...
	logs = y * np.log(softmax(X,W))
	NLL = -np.sum(np.sum(logs, axis = 1)) + reg * np.diag(W.T @ W).sum() #used answers for reg term
	# mu = X @ W # m x k
	# exp_mu = np.exp(mu)
...
def grad_softmax(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	#used solutions for derivative

	return X.T @ (softmax(X,W) - y) + reg*W
	# mu = X @ W
	# exp_mu = np.exp(mu)
	# prob = exp_mu / exp_mu.sum(axis=1).reshape(-1, 1)
...
def get_accuracy(y_pred, y):
	...
	"""
	diff = (y_pred.reshape(-1,1) == y).astype(int)

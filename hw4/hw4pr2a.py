...
def grad_logreg(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	grad = -X.T@(y - sigmoid(X@W)) + 2*reg*W
...
def NLL(X, y, W, reg=0.0):
	"""	This function takes in four arguments:
		...
		HINT:
			...
			4) Use np.log to calculate the log of each entry of the input array

		NOTE: please use the variable given for the final returned result, nll.
	"""

	"*** YOUR CODE HERE ***"
	nll = -np.sum(y*np.log(sigmoid(X@W)) + (1 - y)*np.log(1 - sigmoid(X@W))) + np.linalg.norm(reg*W.T@W)
	# mu = sigmoid(X @ W)
...
def newton_step(X, y, W, reg=0.0):
	...
	S = np.diag((mu*(1-mu)).T[0])
	identity = np.eye(X.T.shape[0])

	H = X.T@S@X + 2*reg*identity
...
def newton_method(X, y, reg=0.0, eps=1e-6, max_iter=20, print_freq=5):
	...
	while iter_num < max_iter and np.linalg.norm(step) > eps:
		...
		nll = NLL(X, y, W, reg)
		if np.isnan(nll):
			break 			#got this from the solution because I was getting weird errors
		nll_list.append(nll)
...
def get_description(X, y, W):
	...
	accuracy = np.sum((y==predictions).astype(int))/y.shape[0]
	precision = np.sum(np.logical_and(predictions, y ).astype(int))/np.sum(predictions==1).astype(int)
	recall = np.sum(np.logical_and(predictions, y).astype(int))/np.sum(y==1).astype(int)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = np.linspace(0, 10, 10, endpoint=True)
	for lambda_ in reg_list:
		#used solution to get the graph to look the same
		a,p,r,f1 = get_description(X_test, y_test, grad_descent(X_train, y_train, reg = lambda_, lr = 2e-4)[0])
		a_list.append(a)
		p_list.append(p)
		r_list.append(r)
		f1_list.append(f1)

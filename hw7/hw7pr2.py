...
def find_cost(X, y, W , reg):
	...
	"*** YOUR CODE HERE ***"
	# cost = (np.linalg.norm(X@W-y, ord = 2)**2 + reg*np.linalg.norm(W, ord = 1))/X.shape[0]
# 	#
# 	# "*** END YOUR CODE HERE ***"
# 	# return cost

	# used this from solution because mine wasn't working
	m = len(y)
	err = X @ W - y
	err = float(err.T @ err)
...
def find_grad(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	# grad = 2*X.T@X@W - (2*y.T@X).T + reg*np.sign(W)
	#
	# "*** END YOUR CODE HERE ***"
	# return grad
...
def grad_lasso(
	...
	max_iter=300, batch_size=1024, print_freq=100):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		mini_batch = np.random.randint(0, m, size=batch_size)
		W_grad = find_grad(X[mini_batch], y[mini_batch], W, reg=reg)
		W = W - lr*W_grad
		# W = prox(W, eps)
...
def lasso_path(X, y, tau_min=1e-8, tau_max=10, num_reg=10):
	...
	for index in range(num_reg):
		...
		# 		as they are.
		"""*** YOUR CODE HERE ***"""
		W_current, obj_list = grad_lasso(X, y, reg=reg, lr=1e-12, \
			max_iter=1000, batch_size=1024, print_freq=1000)
		print(W_current.shape)
		W[:,index] = W_current.flatten()
